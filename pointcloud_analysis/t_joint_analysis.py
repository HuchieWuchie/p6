import sys
sys.path.insert(0, '/home/karl/P6/Code/p6/pointcloud_analysis/include')
...
for i in range(len(line)):
    ...
    curv1max = np.max(curvature)
    res = np.where(curvature == curv1max)
    first_max_idx = res[0][0]

    curv2max = np.max(curvature[0:1000])
    res2 = np.where(curvature == curv2max)
    second_max_idx = res2[0][0]

    linefit_right = linefit(line_x[second_max_idx], line_z[second_max_idx], line_x[0], line_z[0])
    linefit_right_x = linefit_right[0]
    linefit_right_z = linefit_right[1]

    linefit_left = linefit(line_x[first_max_idx], line_z[first_max_idx], line_x[2000], line_z[2000])
    linefit_left_x = linefit_left[0]
    linefit_left_z = linefit_left[1]

    horizontal_line_point_right = np.array([line_x[second_max_idx-100],line_z[second_max_idx]])
    rotateline_right = []

    horizontal_line_point_left = np.array([line_x[first_max_idx+100],line_z[first_max_idx]])
    rotateline_left = []
...
os.chdir("/home/karl/P6/Code/p6/pointcloud_analysis/trajectories")
